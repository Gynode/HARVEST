...
**Deployment is via a `gh-pages` branch, not `actions/deploy-pages`, and the site is now live.** The workflow
builds the site and force-pushes `build/` to `gh-pages` (`7e97f12`, built from `8968651`), and on 2026-09-19
the user repointed **Settings → Pages → Source: "Deploy from a branch" → Branch: `gh-pages` / `(root)`**.
Verified against the live site: `https://gynode.github.io/HARVEST/` serves the Docusaurus build
(`/assets/css/styles.64bc7f80.css`, `/assets/js/main.46f4128f.js`), and `/docs/introduction/`,
`/docs/hrv-token/` and `/docs/status/` all return 200 and render their headings. This branch arrangement was
chosen because GitHub's Pages settings did not offer "GitHub Actions" as a source in this repository.

**Only `build/` is published, which is a change of exposure.** While Pages served `main` / `(root)`, Jekyll
rendered the repository's markdown, so `AGENTS.md` and `CLAUDE.md` were publicly readable at
`/HARVEST/AGENTS.html` and `/HARVEST/CLAUDE.html`. They are not any more, and neither is anything else outside
`build/` — `blockchain/`, the project docs and the internal notes are all off the web.
...
7. **The push to `main`.** Everything is committed; nothing is pushed. `origin/main` is at `77d98a2` and
   three commits sit local on top of it (`38d90d4`, `730a185`, `4d4f979`). `main` runs the deploy workflow, so
   a push publishes — it needs the user's go-ahead. The Pages setting that used to be outstanding is **done**
   (see above).

...
5. **The push to `main` itself.** The Pages workflow is now verified locally — `npm ci && npm run build`
   succeeds on Node 20 and the built site serves — but it has never run in Actions, and `main` is a live
   Pages branch. Nothing has been pushed.
